...
public class ZipExtractorPlugin implements IPlugin {
    ...
    @Inject
    public ZipExtractorPlugin(Metrics.Factory metricsFactory) {
    ...
    public List<? extends ICommandSender> getOnlinePlayers() {
        List<SpongeCommandSender> l = new ArrayList<>();
        game.getServer().getOnlinePlayers().forEach(p -> l.add(new SpongeCommandSender(p)));
        return l;
    }
